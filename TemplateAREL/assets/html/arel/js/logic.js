...
arel.sceneReady(function() {
    ...
    scenario.currentTrackingConfigPathOrIdentifier = "html/resources/TrackingConfig_73512f7275e8b9dfc915fc730c9b9e9c.zip";
    ...
    scenario.onStartup = function () {
        arel.Debug.log("Welcome to the 'propuesta1' Augmented Reality experience.");
    ...
    model5.type = "Model";
    model5.scene = scene1;
    model5.associatedTrackable = pattern1;
    ...
    model5.onTouchStarted = function () {
    ...
    // mail
    if (!arel.Scene.objectExists("model6")) {
        arel.Debug.log("ERROR: retrieving the object model6, it seems that it does not exist. Maybe it would help cleaning the cache or a problem occurred while reloading the channel.");
    }
    ...
    model6.type = "Model";
    model6.scene = scene1;
    model6.associatedTrackable = pattern1;
    ...
    model6.onTouchStarted = function () {
        arel.Debug.log(this.objectName + ".onTouchStarted()");
        website3.display();
    };
    ...
    model7.type = "Model";
    model7.scene = scene1;
    model7.associatedTrackable = pattern2;
    ...
    model7.onTouchStarted = function () {
        arel.Debug.log(this.objectName + ".onTouchStarted()");
        website4.display();
    };
    ...
    var website2 = {};
    website2.objectName = "website2";
    website2.url = "https://twitter.com/afrasilv";
    website2.type = "Website";
    website2.scene = scene1;
    website2.associatedTrackable = null;
    scenario.contents.push(website2);

    website2.setScene = function (scene) {
        this.scene = scene;
        scenario.currentScene.display();
    };

    website2.display = function () {
        arel.Debug.log(this.objectName + ".display() with URL " + this.url);
        if (this.scene && this.scene != scenario.currentScene) {
            return;
        }
        arel.Events.setListener(arel.Media, this.mediaEventsCallback, this);
        arel.Media.openWebsite(this.url, false);
        if (scenario.googleAnalytics) {
            scenario.googleAnalytics.logWebsite("https%3A%2F%2Ftwitter.com%2Fafrasilv");
        }
    };

    website2.mediaEventsCallback = function (eventType, mediaURL) {
        if (eventType == arel.Events.Media.WEBSITE_CLOSED && mediaURL == this.url && methodExists(this, this.onClosed)) {
            this.onClosed();
        }
    };
    ...
    var website3 = {};
    website3.objectName = "website3";
    website3.url = "mailto:afrancodq@alumnos.unex.es";
    website3.type = "Website";
    website3.scene = scene1;
    website3.associatedTrackable = null;
    scenario.contents.push(website3);

    website3.setScene = function (scene) {
        this.scene = scene;
        scenario.currentScene.display();
    };

    website3.display = function () {
        arel.Debug.log(this.objectName + ".display() with URL " + this.url);
        if (this.scene && this.scene != scenario.currentScene) {
            return;
        }
        ...
        arel.Media.openWebsite(this.url, false);
        if (scenario.googleAnalytics) {
            scenario.googleAnalytics.logWebsite("mailto%3Aafrancodq%40alumnos.unex.es");
        }
    };

    website3.mediaEventsCallback = function (eventType, mediaURL) {
        if (eventType == arel.Events.Media.WEBSITE_CLOSED && mediaURL == this.url && methodExists(this, this.onClosed)) {
            this.onClosed();
        }
    };
    ...
    var website4 = {};
    website4.objectName = "website4";
    website4.url = "https://www.museodelprado.es/";
    website4.type = "Website";
    website4.scene = scene1;
    website4.associatedTrackable = null;
    scenario.contents.push(website4);

    website4.setScene = function (scene) {
        this.scene = scene;
        scenario.currentScene.display();
    };

    website4.display = function () {
        arel.Debug.log(this.objectName + ".display() with URL " + this.url);
        if (this.scene && this.scene != scenario.currentScene) {
            return;
        }
        ...
        arel.Media.openWebsite(this.url, false);
        if (scenario.googleAnalytics) {
            scenario.googleAnalytics.logWebsite("https%3A%2F%2Fwww.museodelprado.es%2F");
        }
    };

    website4.mediaEventsCallback = function (eventType, mediaURL) {
        if (eventType == arel.Events.Media.WEBSITE_CLOSED && mediaURL == this.url && methodExists(this, this.onClosed)) {
            this.onClosed();
        }
    };
